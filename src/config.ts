 ...
 * 토큰 주소로 풀 찾기 (프론트 편의)
 */
export function findFarmByToken(
  config: FrontConfig,
  tokenAddress: string
...
export function getTotalRewards(config: BaseConfig, timestamp?: number) {
  ...
  const rewardToTimestamp =
    (config.rewardToTimestamp || timestamp || 0) + SECOND_PER_YEAR;

  const rewardSeconds =
    rewardToTimestamp - rewardFromTimestamp || SECOND_PER_YEAR;
  ...
  if (halvingInterval) {
    ...
    while (localFrom < rewardToTimestamp && halvingNumber < 100) {
      const nextHalvingTime =
        rewardFromTimestamp + (halvingNumber + 1) * halvingInterval;
      const periodEnd =
        rewardToTimestamp < nextHalvingTime
          ? rewardToTimestamp
          : nextHalvingTime;
