...
const fixtureData = {
    ...
    lpDepositAmt: 200,
    lpWithdrawAmt: 50,
    rewardMintAmt: 100000,
    rewardTransferAmt: 50000,
    rewardsPerSecAmt: 1,
};
...
describe('MasterPool + ERC4626Pool', () => {
    async function fixture() {
        ...
        const { lpMintAmt, rewardMintAmt, rewardTransferAmt, rewardsPerSecAmt } = fixtureData;

        const reward = await hre.viem.deployContract('ERC20Mock', ['Reward', 'RWD', 18, 0n]);
        const lp = await hre.viem.deployContract('ERC20Mock', ['LP', 'LP', 18, 0n]);
        const WETH = await hre.viem.deployContract('WETH');

        await lp.write.mint([alice.account.address, parseEther(String(lpMintAmt))]);
        ...
        await reward.write.mint([owner.account.address, parseEther(String(rewardMintAmt))]);
        await reward.write.transfer([vault.address, parseEther(String(rewardTransferAmt))]);
        ...
        await master.write.add([lp.address, 1000, 0 as any, BigInt(2 ** 32), false]);
        const pid = await master.read.poolId([lp.address]);
        const stAddr = await master.read.poolTokens([pid]);
        const st = await hre.viem.getContractAt('ERC4626Pool', stAddr as `0x${string}`);
    ...
    it('예치/인출 시 stTOKEN 민트/번 훅 동작', async () => {
        ...
        const { lpDepositAmt, lpWithdrawAmt } = fixtureData;

        await lp.write.approve([master.address, parseEther(String(lpDepositAmt))], {
            account: alice.account,
        });
        await master.write.deposit([pid, parseEther(String(lpDepositAmt))], {
            account: alice.account,
        });
        expect(await st.read.balanceOf([alice.account.address])).to.equal(parseEther(String(lpDepositAmt)));

        await master.write.withdraw([pid, parseEther(String(lpWithdrawAmt))], {
            account: alice.account,
        });
        expect(await st.read.balanceOf([alice.account.address])).to.equal(
            parseEther(String(lpDepositAmt - lpWithdrawAmt)),
        );
    });
    ...
    it('네이티브 보상 풀: balanceOf가 pending 보상(가상)을 가산해 표시', async () => {
        ...
        await master.write.add([reward.address, 1000, 0 as any, BigInt(2 ** 32), false], {
            account: owner.account,
        });
        const pid2 = await master.read.poolId([reward.address]);
        const st2Addr = await master.read.poolTokens([pid2]);
        const st2 = await hre.viem.getContractAt('ERC4626Pool', st2Addr as `0x${string}`);

        await reward.write.transfer([alice.account.address, parseEther(String(depositAmt))]);
        await reward.write.approve([master.address, parseEther(String(depositAmt))], {
            account: alice.account,
        });
        await master.write.deposit([pid2, parseEther(String(depositAmt))], {
            account: alice.account,
        });
    ...
    it('반감 스케줄(10초마다 50%): 동일 기간 대비 보상이 구간별 감소', async () => {
        ...
        await lp.write.approve([master.address, parseEther(String(depositAmt))], {
            account: alice.account,
        });
        await master.write.deposit([pid, parseEther(String(depositAmt))], {
            account: alice.account,
        });
        ...
        const bal = await (
            await hre.viem.getContractAt('ERC20Mock', (await master.read.rewardToken()) as `0x${string}`)
