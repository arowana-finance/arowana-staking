...
import { parseEther, parseUnits, encodeFunctionData, type Address, type Hex, WalletClient } from 'viem';
...
async function deployTokenAndOracle(owner: WalletClient, tokenConfig?: AddToken) {
    ...
    if (tokenConfig.symbol === 'WETH') {
        ...
        if (!(await hasCode(tokenConfig.address))) {
            ...
            WETH = await hre.viem.getContractAt('WETHMock', tokenConfig.address as Address);
        }
    } else {
        // ERC20
        if (!(await hasCode(tokenConfig.address))) {
            ...
            Token = await hre.viem.getContractAt('ERC20Mock', tokenConfig.address as Address);
        }
    }
    ...
    if (!(await hasCode(tokenConfig.oracle)) && tokenConfig.mockUsdPrice) {
        ...
        Oracle = await hre.viem.getContractAt('DataFeed', tokenConfig.oracle as Address);
    }
...
async function deployTokens(owner: WalletClient, config: AddConfig) {
    ...
    const Tokens: Record<number, ContractTypesMap['ERC20Mock'] | ContractTypesMap['WETHMock']> = {};
    ...
    if (!(await hasCode(config.rewardVault))) {
        ...
        await logTx(
            ...
            RewardVaultProxy.write.initializeProxy(
                ['RewardVault', owner.account!.address, RewardVaultImpl.address, initData],
                { account: owner.account },
            ),
        );
        ...
        await logTx(
            ...
            RewardToken.write.transfer([RewardVault.address, parseEther(String(totalRewards))], {
                account: owner.account,
            }),
        );
    } else {
        RewardVault = await hre.viem.getContractAt('RewardVault', config.rewardVault as Address);
    }
...
async function deployMasterPool(
    ...
    await logTx(
        ...
        MasterPoolProxy.write.initializeProxy(
            ['MasterPool', owner.account!.address, MasterPoolImplementation.address, initData],
            { account: owner.account },
        ),
    );
...
async function addPools(config: AddConfig, timestamp: number, MasterPool: ContractTypesMap['MasterPool']) {
...
async function depositMasterPool(
    ...
    Tokens: Record<number, ContractTypesMap['ERC20Mock'] | ContractTypesMap['WETHMock']>,
) {
    ...
    for (const pool of config.pools || []) {
        ...
        await logTx(
            ...
            MasterPool.write.depositPermit([pool.pid as number, deposit, BigInt(deadline), sig]),
        );
    }
    ...
    for (const pool of config.pools || []) {
        ...
        await logTx(`Withdraw token${pool.pid}`, MasterPool.write.withdraw([pool.pid as number, withdraw]));
    }
}
